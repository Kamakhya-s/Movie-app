import styles from "../MultipleSelection/MultipleSelection.module.css"
import type MultipleSelectionDTO from "./MultipleSelectionDTO.model";


export default function MultipleSection(props: MultipleSelectionProps) {
...
function select(item: MultipleSelectionDTO){
    ...
    props.onChange(nonSelected,selected);
    }
...
return(
    ...
    <ul className={styles.list}>          
        {props.nonSelected.map(item=> <li key={item.key} onClick={()=>select(item)}>{item.description}</li>)}
    </ul>
